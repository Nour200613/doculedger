...
import { FileSpreadsheet, Lock, Mail, Building, ArrowRight } from "lucide-react";
...
export function SignupForm() {
  ...
  const [org, setOrg] = useState("Vance & Cole Accounting LLP");
  const [email, setEmail] = useState("partner@vance-cole.com");
    ...
    <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full py-2.5 px-4 rounded-lg bg-navy-800 hover:bg-navy-900 text-white font-semibold flex items-center justify-center gap-2 shadow-md transition-all mt-4"
        >
          <span>{isLoading ? "Creating account..." : t.signUpBtn}</span>
